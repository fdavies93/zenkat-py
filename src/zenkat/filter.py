...
def parse_filter_expr(tokens: list[str]) -> Tuple[list,list]:
    ...
    for i, token in enumerate(remaining[:4]):
        if token in (')', 'and', 'or'):
            ...
            finish_early = True
            out.extend(remaining[:3])
            remaining = remaining[3:]
    
    if not finish_early:
        out.extend(remaining[:4])
...
def interpret_filter(tokens: list[str], data_type):
...
def interpret_filter_ast(ast: list, data_type):
    '''
    Take an abstract syntax tree generated by parse_filter_expr and generate a compound function representing that filter.
    '''

    operator = ast[-2]
    if operator in ('and', 'or'):
        # it's a compound filter
        l = interpret_filter_ast(ast[0], data_type)
        r = interpret_filter_ast(ast[2], data_type)
        if operator == 'and':
            return lambda o: all([l(o), r(o)])
        elif operator == 'or':
            return lambda o: any([l(o), r(o)])
    else:
        # it's not (a valid one)
        return interpret_filter(ast, data_type)

def parse_filter_str(filter_str: str, data_type):    
    tokens = lex_filter_str(filter_str)
    ast = parse_filter_expr(tokens)[0]
    filter_fn = interpret_filter_ast(ast, data_type)
    return filter_fn
